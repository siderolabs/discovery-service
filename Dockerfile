...
#
# Generated on 2026-09-10T11:08:23Z by kres 3e1d3c0.
...
# runs markdownlint
FROM docker.io/oven/bun:1.4.2-alpine AS lint-markdown
WORKDIR /src
...
RUN --mount=type=cache,target=/root/.cache/go-build,id=discovery-service/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=discovery-service/go/pkg go install github.com/shanduur/dis-vulncheck@${DIS_VULNCHECK_VERSION} \
...
WORKDIR /src
RUN --mount=type=cache,target=/root/.cache/go-build,id=discovery-service/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=discovery-service/go/pkg dis-vulncheck -tool=false ./...
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=discovery-service/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=discovery-service/go/pkg GOARCH=amd64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /discovery-service-linux-amd64
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=discovery-service/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=discovery-service/go/pkg GOARCH=arm64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /discovery-service-linux-arm64
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=discovery-service/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=discovery-service/go/pkg GOARCH=amd64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /snapshot-decoder-linux-amd64
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=discovery-service/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=discovery-service/go/pkg GOARCH=arm64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /snapshot-decoder-linux-arm64
